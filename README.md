...
Inbouwen is één regel; zie **INSTALLEREN.md**.
...
Geen nieuwe dependencies: alleen `express`, dat je al hebt.

## Hoe het is opgebouwd

```
src/sondeertool/
  index.js            de router — dit is het enige dat je require't
  pagina.html         het paginatemplate, met eigen {{placeholders}}
  assets/             stylesheet en client-JS, uitgeleverd door de router zelf
...
De module raakt niets buiten deze map. Er is geen template-engine,
`express.static`-mount, database of omgevingsvariabele nodig; de router
serveert de pagina en de assets zelf.
...
Alle routes zijn relatief aan waar je de router monteert, dus met
`app.use('/bodemcheck', …)` wordt `GET /` de pagina op `/bodemcheck`.
...
| `GET /assets/sondeertool.css` en `.js` | door de router zelf uitgeleverd |

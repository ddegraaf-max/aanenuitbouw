...
(function () {
  ...
  const veld = (naam) => document.querySelector(`[data-veld="${naam}"]`);
  ...
  function zetMelding(tekst, soort) {
    ...
    melding.className = 'melding' + (soort === 'ok' ? ' melding--ok' : '');
    melding.hidden = false;
  }

  function bezig(aan) {
    zoekknop.disabled = aan;
    zoekknop.querySelector('.knop__spinner').hidden = !aan;
    zoekknop.querySelector('.knop__tekst').textContent = aan ? 'Bezig met opzoeken' : 'Bodem opzoeken';
  }
  ...
  function tekenLineaal(maxDiepte, hoogte) {
    ...
    for (let d = 0; d <= maxDiepte; d += stap) {
      ...
      html += `<span class="tik${groot ? ' tik--groot' : ''}" style="top:${(d / maxDiepte) * 100}%">${d}</span>`;
    }
  ...
  function tekenLagen(sondering, maxDiepte) {
    ...
    balk.innerHTML = sondering.lagen
      .map((laag, i) => {
        ...
        const naam = h > 3.2 ? `<span class="laag__naam">${laag.label}</span>` : '';
        return `<div class="laag" title="${laag.label} — ${laag.van} tot ${laag.tot} m, gemiddeld ${laag.qcGemiddeld} MPa"
    ...
    el('lagentabel').innerHTML = sondering.lagen
      .map(
        (l) => `<tr>
          ...
          <td><span class="vlek" style="background:${l.kleur}"></span>${l.label}</td>
  ...
  function tekenSchets(data) {
    ...
    const merken = punten
      .map((p, i) => {
        ...
        return `<g class="merk" data-bro="${p.broId}">
    ...
    el('metinglijst').innerHTML = data.alleLocaties
      ...
      .map(
        (p) => `<li data-actief="${p.geanalyseerd}">
          ...
          <span class="afstand">${p.afstandM} m</span>
        </li>`,
      )
      .join('');
  }
  ...
  if (vraagform) {
    vraagform.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        vraagmelding.className = 'melding' + (res.ok ? ' melding--ok' : '');
